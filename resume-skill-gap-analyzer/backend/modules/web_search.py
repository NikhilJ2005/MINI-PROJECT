...
# Simple in-memory cache: {query_key: {url, title, ts}}
...
def search_learning_resource(skill: str, resource_name: str = "") -> Optional[Dict]:
    ...
    Args:
        ...
        resource_name: Optional specific resource name (e.g., "FreeCodeCamp Docker Tutorial")
...
def enrich_learning_path(learning_path: List[Dict]) -> List[Dict]:
    ...
    replaces hallucinated URLs with real ones. Adds a 'verified' flag.
    ...
    logger.info(f"[WebSearch] Enriched learning path: {verified_count} verified links")
